...
3. **Present tool options**
   ...
   capture the exact installed version. Use the module name from the reference
   table, not the executable or display label. In particular, HD-BET must be
   checked as `hdbet`, not `hd-bet`:
   ```bash
...
4. **Write the SLURM script**
   ...
   **ANTs special case (check first, before writing anything):** If the user
   ...
   - HD-BET → `references/hd-bet.md` and module lookup name `hdbet`
