...
1. **Identify the input image**
   ...
   Read `$ARGUMENTS` for a NIfTI path. If none is given, ask the user:
...
3. **Present tool options**
   ...
   Load `${CLAUDE_PLUGIN_ROOT}/references/tool-comparison.md` and present the
   comparison table to the user. Include columns: Tool, Method, Speed, Template
   Required, GPU Optional, Best For.
...
4. **Write the SLURM script**
   ...
   **ANTs special case (check first, before writing anything):** If the user
   ...
   in `${CLAUDE_PLUGIN_ROOT}/references/ants-brain-extraction.md`:
   ```bash
   ls "<template_path>"
   ```
   ...
   - FSL BET → `${CLAUDE_PLUGIN_ROOT}/references/fsl-bet.md`
   - ANTs → `${CLAUDE_PLUGIN_ROOT}/references/ants-brain-extraction.md`
   - FreeSurfer mri_synthstrip or SynthStrip → `${CLAUDE_PLUGIN_ROOT}/references/synthstrip.md`
   - HD-BET → `${CLAUDE_PLUGIN_ROOT}/references/hd-bet.md`
   - AFNI 3dSkullStrip → `${CLAUDE_PLUGIN_ROOT}/references/afni-3dskullstrip.md`
...
5. **Submit the job**
   ...
   > so, I can wrap the submission using the `/datalad-run` skill. Reply 'yes'
   > to use DataLad, or 'no' to submit directly."

   If yes, hand off to `/datalad-run` before proceeding. If no, submit directly:
   ```bash
...
6. **Wait for completion and hand off to QC**
   ...
   - Invoke `/brain-extraction-qc` with the original image and brain mask:
...
- ALWAYS name scripts `analysis_<N>_brain_extraction.sh` (CLAUDE.md rule).
