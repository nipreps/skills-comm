...
| **HD-BET** | `hdbet/2.0.1` | Deep learning (trained on multi-site data) | Fast on GPU (~2–5 min); Moderate on CPU (~15–20 min) | No | Yes (faster with GPU) | Standard T1w, high-throughput datasets, good default accuracy | Can under-strip in very low-contrast images; CPU mode is slow for large batches |
...
| **SynthStrip standalone** | `synthstrip/7.4.1` with `mri_synthstrip` | Deep learning (same model as mri_synthstrip) | Moderate (~5–15 min) | No | Optional | Any modality, when FreeSurfer is not otherwise needed — lighter dependency | Same as mri_synthstrip; standalone version avoids full FreeSurfer environment overhead |
